...
### 3. Сборка фронтенда (Tailwind CSS / Vite)

В соответствии с Cloud Native подходом, для сборки ассетов не требуется установка Node.js или npm на локальную машину. Сборка выполняется через эфемерный Docker-контейнер.

Выполните эту команду **из корня репозитория** (не из папки `docker/` или `services/`):

**Для Linux / macOS / Windows PowerShell:**

```bash
docker run --rm -v "./services/gateway:/app" -w /app node:22-alpine sh -c "npm install && npm run build"
```

Эта команда скачает зависимости, соберет минифицированные CSS/JS файлы в папку `public/build/` и сразу удалит контейнер.

### 4. Настройка Redis (Кэширование)
...
```json
{
    "original_url": "[https://habr.com/ru/articles/123456/](https://habr.com/ru/articles/123456/)"
}
